...
public interface ChargerRepository extends JpaRepository<ChargerApiEntity, Long> {
    ...
    @Query("SELECT new com.semicolon.springpart.dto.ChargerSearchDTO(c.stationChargerId, c.name, c.chargerType, c.address, c.operatorName, c.output, c.kindDetail, c.updatedAt) FROM ChargerApiEntity c WHERE c.name LIKE %:keyword% OR c.address LIKE %:keyword%")
